...
const requestBasicData = [{
  param: 'URL',
  value: 'http://aiop.bupt.com/restapi/nlp/v1/traditional_2_simplified',
}, {
  param: 'Method',
  value: 'POST',
}, {
...
const requestBodyData = [{
  ...
  description: '待转换成简体的繁体汉字文本（GBK编码的URL编码形式）',
}];

// 请求示例代码
const requestCodeData = `{
    "text": "今天李華到學校旁的咖啡廳喝了兩杯咖啡"
}`;
...
@Form.create()
export default class Traditional2Simplified extends React.Component {
  // 清空调用结果框中内容
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields({ force: true },
      (err, values) => {
        if (!err) {
          this.props.dispatch({ type: 'instruction/invokeTraditional2Simplified', payload: values });
        }
      }
    );
  }
