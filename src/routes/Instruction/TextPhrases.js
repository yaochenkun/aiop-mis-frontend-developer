...
const requestBasicData = [{
  param: 'URL',
  value: 'http://aiop.bupt.com/restapi/nlp/v1/text_phrases',
}, {
  param: 'Method',
  value: 'POST',
}, {
...
const requestBodyData = [{
  ...
  description: '待提取短语的文本（GBK编码的URL编码形式）',
}, {
  param: 'size',
  type: 'int',
  mandatory: <Icon type="close" />,
  description: '希望提取的短语个数（默认为3）',
}];
...
const responseBodyData = [{
  ...
  param: 'index',
  type: 'int',
  description: '短语在句中的索引位置, 若为融合短语则返回-1',
}];
...
@Form.create()
export default class TextPhrases extends React.Component {
  // 清空调用结果框中内容
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields({ force: true },
      (err, values) => {
        if (!err) {
          this.props.dispatch({ type: 'instruction/invokeTextPhrases', payload: values });
        }
      }
    );
  }
