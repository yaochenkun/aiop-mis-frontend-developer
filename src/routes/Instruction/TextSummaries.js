...
const requestBasicData = [{
  param: 'URL',
  value: 'http://aiop.bupt.com/restapi/nlp/v1/text_summaries',
}, {
  param: 'Method',
  value: 'POST',
}, {
...
const requestBodyData = [{
  ...
  description: '待提取摘要的文本（GBK编码的URL编码形式）',
}, {
  param: 'size',
  type: 'int',
  mandatory: <Icon type="close" />,
  description: '希望提取的摘要的个数（默认为3）',
}];
...
const responseCodeData = `{
    ...
    "items": [
        {
            "index": 62,
            "item": "提出一种强鲁棒性自适应活性边表算法.
                     新算法引入横度和纵度的概念表示横纵扫描方向上的求交次数"
        },
        {
            "index": -1,
            "item": "深入挖掘不同扫描方向上的求交次数及多边形自交特性"
        },
        {
            "index": 0,
            "item": "为提升多边形的填充效率"
        ...
        {
            "index": 151,
            "item": "正确而高效地填充了自交多边形.经实验验证"
        },
        {
            "index": 172,
            "item": "新算法灵活的自适应性和高效的自交纠正方法"
        }
    ]
}`;
...
@Form.create()
export default class TextSummaries extends React.Component {
  // 清空调用结果框中内容
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields({ force: true },
      (err, values) => {
        if (!err) {
          this.props.dispatch({ type: 'instruction/invokeTextSummaries', payload: values });
        }
      }
    );
  }
