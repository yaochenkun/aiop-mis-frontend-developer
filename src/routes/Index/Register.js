...
export default class Register extends Component {
  ...
  //     this.props.dispatch(routerRedux.push('/index/register_result'));
  //   }
  // }
    ...
    this.props.form.validateFields(['mobile'], { force: true },
      (err, values) => {
        if (!err) {
          ...
          // 请求发送验证码
          this.props.dispatch({ type: 'register/sendCaptcha', payload: values });
        }
      }
    );
  }
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields({ force: true },
      (err, values) => {
        if (!err) {
          this.props.dispatch({ type: 'register/submit', payload: values });
        }
      }
    );
  }
    ...
    return (
      <div className={styles.main}>
        ...
        <Form onSubmit={this.handleSubmit}>
          <FormItem>
            {getFieldDecorator('username', { rules: [{ required: true, message: '请输入账户名' }, { pattern: REGEX.USERNAME, message: '账户名由不小于8位的字母或数字组成' }] })(
            ...
            <InputGroup size="large" className={styles.mobileGroup} compact>
              ...
              <FormItem style={{ width: '80%' }}>
                {getFieldDecorator('mobile', {
                  rules: [{ required: true, message: '请输入手机号！' }, { pattern: REGEX.PHONE, message: '手机号格式错误！' }],
                })(
          ...
          <FormItem>
            ...
            <Link className={styles.login} to="/index/login">使用已有账户登录</Link>
          </FormItem>
        </Form>
      </div>
    );
